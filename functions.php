 ...
 */
if(!function_exists('strFirstUpEbbs')) {
    function strFirstUpEbbs($str) {
 ...
 */
if(!function_exists('dateToTimestampEbbs')) {
    function dateToTimestampEbbs($date) {
        if(empty($a))
            return false;
        $a = explode(EBBS_DATE_DL, $date);
 ...
 */
if(!function_exists('getRandNameEbbs')) {
    function getRandNameEbbs($lenFrom = 6, $lenTo = 9) {
        $res = '';
...
if(!function_exists('importClassEbbs')) {
    function importClassEbbs($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), EBBS_CODE) !== false) {
					$classFile = preg_replace('/'. EBBS_CODE. '$/i', '', $classFile);
				}
                $path = EBBS_CLASSES_DIR. $classFile. '.php';
			}
            return import($path);
        }
 ...
 */
if(!function_exists('toeGetClassNameEbbs')) {
    function toeGetClassNameEbbs($class) {
        $className = '';
		if(class_exists($class. strFirstUpEbbs(EBBS_CODE)))
			$className = $class. strFirstUpEbbs(EBBS_CODE);
		else if(class_exists(EBBS_CLASS_PREFIX. $class))
            $className = EBBS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjEbbs')) {
    function toeCreateObjEbbs($class, $params) {
        $className = toeGetClassNameEbbs($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectEbbs')) {
    function redirectEbbs($url) {
        if(headers_sent()) {
...
if(!function_exists('prepareFieldCodeEbbs')) {
	function prepareFieldCodeEbbs($string) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsEbbs')) {
	function prepareParamsEbbs(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeEbbs($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
