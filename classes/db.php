...
class dbEbbs {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see EBBS_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, EBBS_DB_PREF, $wpdb->prefix. EBBS_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampEbbs($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, EBBS_DATE_DL)) return dateToTimestampEbbs($date);
        $arr = explode('-', $date);
        return dateToTimestampEbbs($arr[2]. EBBS_DATE_DL. $arr[1]. EBBS_DATE_DL. $arr[0]);
    }
