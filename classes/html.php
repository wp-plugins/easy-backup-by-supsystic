...
class htmlEbbs {
        ...
        return self::input($name, array(
            ...
            // <!--
                jQuery(document).ready(function(){
                    jQuery("#'.$id.'").datepicker({dateFormat: "'.EBBS_DATE_FORMAT_JS.'",
                        changeYear: true,
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = EBBS_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
        $out = '';
        if(strpos($params['url'], 'pl='. EBBS_CODE) === false)
			$params['url'] = uriEbbs::_(array('baseUrl' => $params['url'], 'pl' => EBBS_CODE));
        $out .= self::button(array('value' => __( empty($params['buttonName']) ? 'Upload' :  $params['buttonName'] , EBBS_LANG_CODE), 'attrs' => 'id="toeUploadbut_'.$name.'" class="button button-large"'));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterEbbs::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsEbbs::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterEbbs::getCountries($params['notSelected']);
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlEbbs::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsEbbs::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label'], EBBS_LANG_CODE). htmlEbbs::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = frameEbbs::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			foreach(fieldAdapterEbbs::getFontsList() as $font)
				$options[ $font ] = $font;
		}
