<?php
abstract class moduleEbbs extends baseObjectEbbs {
    /**
     * @var controllerEbbs $_controller
     */
	...
	public function __construct($d, $params = array()) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsEbbs::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsEbbs::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return EBBS_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return EBBS_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			...
			if(import($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameEbbs($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassEbbs($helper, $this->getModDir(). 'helper.php');
	...
	public function __call($name, $arguments) {
		...
		errorsEbbs::push(__(array('Module', $this->_code, 'method', $name, 'undefined'), EBBS_LANG_CODE), errorsEbbs::FATAL);
	}
