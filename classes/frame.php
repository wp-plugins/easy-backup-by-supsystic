...
class frameEbbs {
    ...
    public function __construct() {
        $this->_res = toeCreateObjEbbs('response', array());

    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameEbbs();
        }
        return $instance;
    }
    ...
    public function parseRoute() {
        // Check plugin
        $pl = reqEbbs::getVar('pl');
        if($pl == EBBS_CODE) {
            $mod = reqEbbs::getMode();
            if($mod)
                $this->_mod = $mod;
            $action = reqEbbs::getVar('action');
            if($action)
                $this->_action = $action;
        }
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsEbbs::getExtModDir( $m['ex_plug_dir'] );
                }

                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassEbbs($code. strFirstUpEbbs(EBBS_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameEbbs($code);
    ...
    public function init() {
        ...
        register_uninstall_hook(EBBS_DIR. DS. EBBS_MAIN_FILE, array('utilsEbbs', 'deletePlugin'));
        register_activation_hook( EBBS_DIR. DS. EBBS_MAIN_FILE, array('utilsEbbs', 'activatePlugin')  ); //See classes/install.php file

        add_action('admin_notices', array('errorsEbbs', 'displayOnAdmin'));
    ...
    public function connectLang() {
        load_plugin_textdomain(EBBS_LANG_CODE, false, EBBS_PLUG_NAME. '/lang');
    }
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', EBBS_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[EBBS_METHODS])
                    && !empty($permissions[EBBS_METHODS])

                ) {
                    foreach($permissions[EBBS_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[EBBS_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[EBBS_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = $this->getCurrentUserPosition();
                        if((is_array($permissions[ EBBS_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ EBBS_METHODS ][ $action ]))
                            || (!is_array($permissions[ EBBS_METHODS ][ $action ]) && $permissions[EBBS_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[EBBS_USERLEVELS])
                    && !empty($permissions[EBBS_USERLEVELS])
                ) {
                    $currentUserPosition = $this->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin()) {
						$currentUserPosition = EBBS_ADMIN;
					}
                    foreach($permissions[EBBS_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = EBBS_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = EBBS_TABLES_DIR) {
        importClassEbbs('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableEbbs::_($tableName);
     ...
     * @return tableEbbs table
     * @example frameEbbs::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriEbbs::_($src);
    ...
    public function isTplEditor() {
        $tplEditor = reqEbbs::getVar('tplEditor');
    ...
    {
        return (isset($_GET['page']) && $_GET['page'] === EBBS_PLUGIN_PAGE_URL_SUFFIX);
    }
    ...
    public static function devlog($data) {
        if (is_array($data)) {
            array_map(array('frameEbbs', 'devlog'), $data);
            return;
        }
	...
	public function savePluginActivationErrors() {
		update_option(EBBS_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(EBBS_CODE. '_plugin_activation_errors');
	}
}
