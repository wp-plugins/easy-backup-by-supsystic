...
class langEbbs {
	...
	static public function extract($d = array('dir' => '', 'langEbbs' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langEbbs']) && !empty($d['langEbbs']))
			$langDirPath = EBBS_LANG_DIR. $d['langEbbs']. DS;
		else
			$langDirPath = EBBS_LANG_DIR. EBBS_WPLANG. DS;
    ...
    {
        return __($msgid, EBBS_LANG_CODE);
	}
