...
class fieldAdapterEbbs {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterEbbs', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsEbbs'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsEbbs::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsEbbs::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameEbbs::_()->getModule('products')->getCategories();
        $brands = frameEbbs::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', EBBS_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', EBBS_LANG_CODE). '</b><br />
                '. __('Destination', EBBS_LANG_CODE). ':'. htmlEbbs::checkboxlist('params[dest]', array('optionsEbbs' => $listOfDest)). '<br />
                '. __('Country', EBBS_LANG_CODE). ':'. htmlEbbs::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', EBBS_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', EBBS_LANG_CODE) : htmlEbbs::checkboxlist('params[categories][]', array('optionsEbbs' => $cOptions))). '<br />
                    <b>'. __('Brands', EBBS_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', EBBS_LANG_CODE) : htmlEbbs::checkboxlist('params[brands][]', array('optionsEbbs' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', EBBS_LANG_CODE).': '. htmlEbbs::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', EBBS_LANG_CODE).': '. htmlEbbs::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', EBBS_LANG_CODE) : __($notSelected, EBBS_LANG_CODE);
			}
    ...
    static public function getStates($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if($notSelected) {
				...
				$options[ $notSelected ][0] = array('name' => __( $notSelectedLabel, EBBS_LANG_CODE), 'country_id' => NULL);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameEbbs::_()->getModule('optionsEbbs')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key, EBBS_LANG_CODE).':<br />';
                $output .= htmlEbbs::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', EBBS_LANG_CODE).':<br />';
                $output .= htmlEbbs::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', EBBS_LANG_CODE).':<br />';
                $output .= htmlEbbs::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsEbbs']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsEbbs'][0] = in_array(0,$field->value['categories'])?__('Deselect All', EBBS_LANG_CODE):__('Select All', EBBS_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsEbbs'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameEbbs::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameEbbs::_()->getTable('states')
				->leftJoin( frameEbbs::_()->getTable('countries'), 'country_id' )
