...
class validatorEbbs {
    ...
    static public function validate($field, $label = '', $validate = array()) {
        self::$errors = array();
        if(is_object($field) && get_class($field) != 'fieldEbbs') {
            $value = $field;
            $field = new fieldEbbsEbbs('noMatter');
            $field->label = $label;
        ...
        if(!empty($field->validate)) {
            foreach($field->validate as $v) {
                 if(method_exists('validatorEbbs', $v))
                    self::$v($field);
            }
        }
        if(method_exists('validatorEbbs', $field->type)) {
            $validate = $field->type;
            self::$validate($field);
        }
        if($field->maxlen) {
    ...
    static public function validLen($field, $label = '', $validate = array()) {
        if( !(bool) (strlen($field->value) <= $field->maxlen)) {
			self::addError(__(array('Invalid length for', $field->label, ', max length is', $field->maxlen), EBBS_LANG_CODE), $field->name);
			return false;
		}
		return true;
    }
    ...
    static public function numeric($field) {
        if(!is_numeric($field->value) && !empty($field->value)) {
            self::addError(__(array('Invalid numeric value for', $field->label), EBBS_LANG_CODE), $field->name);
            return false;
        }
        return true;
    }
    ...
    static public function notEmpty($field) {
        if(!self::_notEmpty($field->value)) {
            self::addError(__(array('Please enter', $field->label), EBBS_LANG_CODE), $field->name);
            return false;
        }
        return true;
    }
    static public function selectNotEmpty($field) {
        if(empty($field->value)) {
            self::addError(__(array('Please select', $field->label), EBBS_LANG_CODE), $field->name);
            return false;
        }
        return true;
    }
    static public function email($field) {
        if(!is_email($field->value)) {
            self::addError(__(array('Invalid', $field->label), EBBS_LANG_CODE), $field->name);
            return false;
        } elseif(email_exists($field->value)) {
            self::addError(__(array($field->label, 'is already registered'), EBBS_LANG_CODE), $field->name);
            return false;
        }
        return true;
    }
    ...
    static public function getProductValidationMethods() {
        $res = array();
        $all = get_class_methods('validatorEbbs');
        foreach($all as $m) {
            if(in_array($m, array('int', 'none', 'string'))) {
                $res[$m] = __($m, EBBS_LANG_CODE);
            }
        }
        return $res;
    }
