...
abstract class tableEbbs {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUpEbbs($table). strFirstUpEbbs(EBBS_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjEbbs('fieldEbbs', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbEbbs::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbEbbs::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbEbbs::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothig to update', EBBS_LANG_CODE));
            return false;
        }
        ...
        if(dbEbbs::query($query)) {
            if($method == 'INSERT')
                return dbEbbs::lastID();
            else
                return true;
        } else
			$this->_addError(__('Database error. Please contact your developer.', EBBS_LANG_CODE));
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapterEbbs::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapterEbbs::DB);
                    if($validate) {
                        if (is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validatorEbbs::_($objForValidation)) {
    ...
    /**
     * Add new fieldEbbsEbbs for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjEbbs('fieldEbbs', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbEbbs::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(EBBS_DATE_FORMAT, dbEbbs::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbEbbs::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
