...
class fieldEbbs {
	 ...
	 */
	public $adapt = array('htmlEbbs' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterEbbs::_($value, $this->adapt['dbFrom'], fieldAdapterEbbs::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists(html, $this->html)) {
			...
			if($this->adapt['htmlEbbs']) {
				fieldAdapterEbbs::_($this, $this->adapt['htmlEbbs'], fieldAdapterEbbs::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameEbbs::_()->getModule('optionsEbbs')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', EBBS_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsEbbs'] = $options;
				}
			}
			$htmlContent = htmlEbbs::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlEbbs::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterEbbs::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterEbbs::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsEbbs');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameEbbs::_()->getModule('optionsEbbs')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsEbbs'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlEbbs', $method)) {
			$output .= htmlEbbs::$method($name, $params);
			$output .= htmlEbbs::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', EBBS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', EBBS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', EBBS_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterEbbs::_($id,'getExtraFieldOptions',fieldAdapterEbbs::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', EBBS_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', EBBS_LANG_CODE).':<br />';
					$output .= htmlEbbs::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', EBBS_LANG_CODE).':<br />';
					$output .= htmlEbbs::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterEbbs::_($this, $this->adapt['htmlParams'], fieldAdapterEbbs::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', EBBS_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', EBBS_LANG_CODE).':<br />';
				$output .= fieldAdapterEbbs::_($params,'getFieldAttributes',  fieldAdapterEbbs::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsEbbs::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperEbbs');
		  $module = $this->checkVarFromParam($param,'moduleEbbs');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameEbbs::_()->getModule($module))
			return false; 
		// check for xml file with params structure  
	   if(frameEbbs::_()->getModule($module)->isExternal())
		   $config_xml = frameEbbs::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = EBBS_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', EBBS_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameEbbs::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsEbbs'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 $output .= '<label>'.__($configOptions[$key]['label'], EBBS_LANG_CODE);
					 if ($configOptions[$key]['description'] != '') {
						 $output .= '<a class="toeOptTip" tip="'.__($configOptions[$key]['description'], EBBS_LANG_CODE).'"></a>';
					 }
					 $output .= '</label><br />';
					 $output .= htmlEbbs::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
