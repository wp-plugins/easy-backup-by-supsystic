...
class utilsEbbs {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsEbbs::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsEbbs::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsEbbs::getCurrentWPThemeCode(). DS;
        }
        return $themePath;
    }
    /**
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameEbbs::_()->getModule($m))
                return false;
            if(!is_dir(frameEbbs::_()->getModule($m)->getModDir()))
                return false;
        }
        return true;*/
        foreach(frameEbbs::_()->getModules() as $m) {
	...
	static  public function activatePlugin() {
		...
		if(EBBS_TEST_MODE) {
			add_action('activated_plugin', array(frameEbbs::_(), 'savePluginActivationErrors'));
		}
		installerEbbs::init();
    }
