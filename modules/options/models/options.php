<?php
class optionsModelEbbs extends modelEbbs {
			...
			foreach($d['opt_values'] as $code => $value) {
				...
				if ($code == 'email') {
					if (!$this->validateEmail($value)) {
                        $this->pushError(__('Incorrect email address', EBBS_LANG_CODE));
                        continue;
                    }
                }
				$this->save($d);
				if ($code == 'full') {
					//frameEbbs::_()->getTable('options')->update(array('value' => 1), array('code' => 'database')); //outdated
				}
			}

			return !$this->haveErrors();
		} else
			$this->pushError(__('No change', EBBS_LANG_CODE));
	}

	public function set($value, $code){
        frameEbbs::_()->getTable('options')->update(array('value' => $value), array('code' => $code));
	}

	public function getEvery(){
        return frameEbbs::_()->getTable('options')->get('*', "value_type = 'every'", EBBS_WPDB_PREF.EBBS_DB_PREF.'options', 'all');
	}
	...
	public function saveMainFromDestGroup($d = array()) {
		if (isset($d['dest_opt']) && !empty($d['dest_opt'])){
			...
			if (utilsEbbs::checkPRO() || $d['dest_opt'] == 0){
				$this->set($d['dest_opt'], 'glb_dest');
			} else {
				$this->pushError(__('PRO version is not activated', EBBS_LANG_CODE));
			}
			return !$this->haveErrors();
		} else
			$this->pushError(__('No selected options', EBBS_LANG_CODE));
	}
...
// ---- old -----
	...
	public function getValueType($d) {
		$ret = frameEbbs::_()->getTable('options')->get('value_type', "code = '".$d."'", EBBS_WPDB_PREF.EBBS_DB_PREF.'options', 'all');
		return $ret[0]['value_type'];
	}
	public function setValueType($code, $value) {
		frameEbbs::_()->getTable('options')->update(array('value_type' => $value), array('code' => $code));
	}
    ...
    protected function _loadOptions() {
        if(empty($this->_allOptions)) {
            $options = frameEbbs::_()->getTable('options');
            $htmltype = frameEbbs::_()->getTable('htmltype');
			$optionsCategories = frameEbbs::_()->getTable('options_categories');
            ...
            foreach($this->_allOptions as $i => $opt) {
                if(!empty($this->_allOptions[$i]['params'])) {
                    $this->_allOptions[$i]['params'] = utilsEbbs::unserialize($this->_allOptions[$i]['params']);
                }
				if($this->_allOptions[$i]['value_type'] == 'array') {
					$this->_allOptions[$i]['value'] = utilsEbbs::unserialize($this->_allOptions[$i]['value']);
		...
		if(isset($d['opt_values']) && is_array($d['opt_values']) && !empty($d['opt_values'])) {
			...
			if($id) {
				...
				if($checkArr == 'array' && !empty($checkArr)) {
					$updateData['value'] = utilsEbbs::serialize( $updateData['value'] );
				}
				if(frameEbbs::_()->getTable('options')->update($updateData, array('id' => $id))) {
					// Let's update data in current options params to avoid reload it from database
					if(isset($d['code']))
						$this->_setByCode($d['code'], $d['opt_values'][ $d['code'] ]);
					return true;
				} else
					$this->pushError(__('Option '. $d['code']. ' update Failed', EBBS_LANG_CODE));
			} else {
				$this->pushError(__('Invalid option ID or Code', EBBS_LANG_CODE));
			}
		} else
			$this->pushError(__('Empty data to save option', EBBS_LANG_CODE));
        return false;
    }
	public function saveCodeVal($code, $val) {
		if(frameEbbs::_()->getTable('options')->exists($code, 'code')) {
			frameEbbs::_()->getTable('options')->update(array(
				'value' => $val,
			), array('code' => $code));
			$this->_setByCode($code, $val);
		} else {
			frameEbbs::_()->getTable('options')->insert(array(
				'code' => $code,
				'value' => $val,
			));
		}
	...
	/*public function saveGroup($d = array()) {
		if(isset($d['opt_values']) && is_array($d['opt_values']) && !empty($d['opt_values'])) {
			...
			$this->pushError(__('Empty data to setup', EBBS_LANG_CODE));
	}*/
	public function saveBgImg($d = array()) {
		if(!empty($d) && isset($d['bg_image']) && !empty($d['bg_image'])) {
			$uploader = toeCreateObjEbbs('fileuploader', array());
			if($uploader->validate('bg_image', frameEbbs::_()->getModule('options')->getBgImgDir()) && $uploader->upload()) {
				// Remove prev. image
				utilsEbbs::deleteFile( frameEbbs::_()->getModule('options')->getBgImgFullDir() );
	...
	public function saveLogoImg($d = array()) {
		if(!empty($d) && isset($d['logo_image']) && !empty($d['logo_image'])) {
			$uploader = toeCreateObjEbbs('fileuploader', array());
			if($uploader->validate('logo_image', frameEbbs::_()->getModule('options')->getLogoImgDir()) && $uploader->upload()) {
				// Remove prev. image
				utilsEbbs::deleteFile( frameEbbs::_()->getModule('options')->getLogoImgFullDir() );
				$fileInfo = $uploader->getFileInfo();
				// Save info for this option
	...
	public function setTplDefault($d = array()) {
		...
		if(!empty($code)) {
			...
			if($plTemplate && frameEbbs::_()->getModule($plTemplate)) {
				$newValue = frameEbbs::_()->getModule($plTemplate)->getDefOptions($code);
				if($newValue !== NULL) {
					if($this->save(array('opt_values' => array($code => $newValue), 'code' => $code))) {
						return $newValue;
					}
				} else
					$this->pushError(__('There is no default for this option and current template', EBBS_LANG_CODE));
			} else
				$this->pushError(__('There is no default for this option and current template', EBBS_LANG_CODE));
		} else
			$this->pushError(__('Empty option code', EBBS_LANG_CODE));
		return false;
	}
	public function setBgImgDefault($d = array()) {
		...
		if(!empty($code)) {
			...
			if($plTemplate && frameEbbs::_()->getModule($plTemplate)) {
				...
				if($newValue !== NULL && file_exists(frameEbbs::_()->getModule($plTemplate)->getModDir(). $newValue)) {
					// Remove prev. image
					utilsEbbs::deleteFile( frameEbbs::_()->getModule('options')->getBgImgFullDir() );
					// Copy new image from tpl module directory to uploads dirctory
					copy( frameEbbs::_()->getModule($plTemplate)->getModDir(). $newValue, utilsEbbs::getUploadsDir(). DS. $this->getModule()->getBgImgDir(). DS. $newValue);
					if($this->save(array('opt_values' => array($code => $newValue), 'code' => $code))) {
	...
	public  function removeBgImg($d = array()) {
		$bgImgDirPath = frameEbbs::_()->getModule('options')->getBgImgFullDir();
		if($this->save(array('opt_values' => array('bg_image' => ''), 'code' => 'bg_image'))
			&& utilsEbbs::deleteFile( $bgImgDirPath )
		) {
			return true;
		} else
			$this->pushError(__('Unable to remove image', EBBS_LANG_CODE));
	}
		...
		if(!empty($code)) {
			...
			if($plTemplate && frameEbbs::_()->getModule($plTemplate)) {
				...
				if($newValue !== NULL && file_exists(frameEbbs::_()->getModule($plTemplate)->getModDir(). $newValue)) {
					...
					utilsEbbs::deleteFile( frameEbbs::_()->getModule('options')->getLogoImgFullDir() );
					// Copy new image from tpl module directory to uploads dirctory
					copy( frameEbbs::_()->getModule($plTemplate)->getModDir(). $newValue, utilsEbbs::getUploadsDir(). DS. $this->getModule()->getLogoImgDir(). DS. $newValue);
					if($this->save(array('opt_values' => array($code => $newValue), 'code' => $code))) {
	...
	public function removeLogoImg($d = array()) {
		$logoImgDirPath = frameEbbs::_()->getModule('options')->getLogoImgFullDir();
		if($this->save(array('opt_values' => array('logo_image' => ''), 'code' => 'logo_image'))
			&& utilsEbbs::deleteFile( $logoImgDirPath )
		) {
			return true;
		} else
			$this->pushError(__('Unable to remove image', EBBS_LANG_CODE));
	...
	public function setTitleParamsDefault($d = array()) {
		...
		$plTemplate = $this->get('template');		// Current plugin template
		if($plTemplate && frameEbbs::_()->getModule($plTemplate)) {
			$msgTitleColor = frameEbbs::_()->getModule($plTemplate)->getDefOptions('msg_title_color');
			if($msgTitleColor !== NULL) {
				$this->save(array('opt_values' => array('msg_title_color' => $msgTitleColor), 'code' => 'msg_title_color'));
			}
			$msgTitleFont = frameEbbs::_()->getModule($plTemplate)->getDefOptions('msg_title_font');
	...
	public function setTextParamsDefault($d = array()) {
		...
		$plTemplate = $this->get('template');		// Current plugin template
		if($plTemplate && frameEbbs::_()->getModule($plTemplate)) {
			$msgTextColor = frameEbbs::_()->getModule($plTemplate)->getDefOptions('msg_text_color');
			if($msgTextColor !== NULL) {
				$this->save(array('opt_values' => array('msg_text_color' => $msgTextColor), 'code' => 'msg_text_color'));
			}
			$msgTextFont = frameEbbs::_()->getModule($plTemplate)->getDefOptions('msg_text_font');
			if($msgTextFont !== NULL) {
    ...
    public function saveBackupPath($newBackupPathArray) {
        $backupsPath = frameEbbs::_()->getTable('options')->get('value', array('code' => 'serialized_backups_path'), '', 'row');
        ...
        if(is_array($backupsPath)) {
            ...
            if(!$newPathExist) {
                ...
                frameEbbs::_()->getTable('options')->update(array('value' => serialize($backupsPath)), array('code' => 'serialized_backups_path'));
            }
