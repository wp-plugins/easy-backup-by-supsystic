...
class backupModelEbbs extends modelEbbs {
    ...
    public function getBackupsList() {
        ...
        foreach ($dir as $file) {
            ...
            $fileSize = (file_exists($config['warehouse'] . $file)) ? filesize($config['warehouse'] . $file) : __('Unknown', EBBS_LANG_CODE);
            ...
            if (!empty($backupInfo)) {
                ...
                $backups[$backupInfo['id']]['ftp'][strtolower($backupInfo['ext'])] = dispatcherEbbs::applyFilters('addInfoIfEncryptedDb', $backups[$backupInfo['id']]['ftp'][strtolower($backupInfo['ext'])]);
            }
        }
        krsort($backups);
        return $backups;
