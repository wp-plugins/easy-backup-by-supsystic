...
class filesystemModelEbbs extends modelEbbs {
    ...
    public function create($filename)
    {
        ...
        $warehouse = frameEbbs::_()->getModule('warehouse')->getTemporaryPath();
        if(frameEbbs::_()->getModule('options')->get('warehouse_abs') == 0)
    ...
    public function restore($filename)
    {
        ...
        $absolutePath = frameEbbs::_()->getModule('options')->get('warehouse_abs') ? false : true;
        if (!file_exists($filename)) {
            $this->pushError(sprintf(__('Filesystem backup %s does not exists', EBBS_LANG_CODE), basename($filename)));
            return false;
        }
        ...
        if ($files = $pcl->extract(PCLZIP_OPT_PATH, $absPath, PCLZIP_OPT_REPLACE_NEWER) === 0) {
            $this->pushError(__('An error has occurred while unpacking the archive', EBBS_LANG_CODE));
            return false;
        }
        ...
        // Unpack stacks
        $warehouse = frameEbbs::_()->getModule('options')->get('warehouse');
...
//        $absPath = str_replace('/', DS, ABSPATH);
    ...
    public function getFilesListByBUPDirSettingArray($options)
    {
        ...
        $warehouseDir = 'easy-backup-storage'; // this value writing in 'options' only in plugin installation process
        ...
        if (0 == $isFull) {
            $directory = trailingslashit($directory) . EBBS_WP_CONTENT_DIR;
        }
    ...
    private function clearTmpDirectory(){
        $tmpPath = untrailingslashit(frameEbbs::_()->getModule('warehouse')->getPath()) . DS . 'tmp' . DS;
