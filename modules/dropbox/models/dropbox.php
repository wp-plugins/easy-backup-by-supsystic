 ...
 * @version 1.2
 */
class dropboxModelEbbs extends modelEbbs {
	...
	public function isSupported() {
		if(!defined('EBBS_DROPBOX_SUPPORT')) {
			return false;
		}

		return EBBS_DROPBOX_SUPPORT;
	}
	...
	public function getCredential($key) {
		...
		return frameEbbs::_()->getModule('options')->get($key);
	}
    ...
    public function getBackupsPath() {
        if($this->backupsPath === null) {
            $this->backupsPath = frameEbbs::_()->getModule('warehouse')->getPath()
				. DIRECTORY_SEPARATOR;
        }
	...
	public function download($filename, $returnDataString = false) {
		...
		try {
			...
			if($result === null) {
				$this->pushError(__('File not found', EBBS_LANG_CODE));
				return false;
			}
