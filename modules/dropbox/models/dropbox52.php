...
class dropbox52ModelEbbs extends modelEbbs {
    ...
    public function upload($files = array()) {
        ...
        foreach ($files as $file) {
            ...
            if (file_exists($file = rtrim($filepath, '/') . '/' . $file)) {
                ...
                try {
                    $request = curlEbbs::createPutRequest(
    ...
    public function remove($filepath) {
        ...
        $request = curlEbbs::createPostRequest($url, array(
            'root'   => 'sandbox',
            'path'   => $filepath,
            'locale' => 'en',
        ));
    ...
    public function download($filename, $returnDataString = false) {
        ...
        $request = curlEbbs::createGetRequest(
        ...
        if (!file_put_contents($this->getBackupsPath() . $filename, $response)) {
            $this->pushError(__(sprintf('Can\'t download the file: %', $filename), EBBS_LANG_CODE));
            return false;
        }
    ...
    public function getBackupsPath() {
        return frameEbbs::_()->getModule('warehouse')->getPath()
            . DIRECTORY_SEPARATOR;
    ...
    {
        $storage = frameEbbs::_()->getModule('warehouse')->getPath();
