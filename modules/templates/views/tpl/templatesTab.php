...
// <!--
    ...
    function toeUpdateTemplate(newValue) {
        jQuery(this).sendForm({
            data: {
                reqType: 'ajax', action: 'putOption', page: 'optionsEbbs', code: 'default_theme', value: newValue
            },
            msgElID: 'toeTplMsg',
            onSuccess: function(res) {
                if(!res.error) {
                    jQuery('#toeTplMsg').html('<?php _e('Template Updated. Reload page to see results.', EBBS_LANG_CODE)?>');
...
?>
                <td width="<?php echo ceil(100/$tplPerRow)?>%">
                    <div class="toeTplOption">
                        ...
                        <a href="<?php echo $code?>" class="toeTplActivateLink"><?php _e('Activate', EBBS_LANG_CODE)?></a>
                        <a href="<?php echo $code?>" class="toeTplDeactivateLink"><?php _e('Deactivate', EBBS_LANG_CODE)?></a><br /><br />
